#!/bin/bash
### Hardening Script for RHEL 7 Servers.

#  Remove non used system accounts

echo "AMX Hardening v1.0-rc2 2020-02-13_18:07"
...
echo "##########################################################################################"
echo "PCI-DSS-Req 2.1 - disable default usernames"
echo "##########################################################################################"

for i in bin daemon adm lp sync shutdown halt mail games ftp nobody avahi-autoipd tss postfix systemd-bus-proxy systemd-network operator dockerroot tcpdump insights nxautomation; do 
     userdel -r $i ;
done
...
# don't shcedule on prod, enroll more sysadmins to your team!
systemctl stop atd
systemctl disable atd
# m$ agent
systemctl stop waagent
systemctl disable waagent
# security over adm performance
systemctl stop tuned
systemctl disable tuned
# on cloud servers
systemctl stop smartd
systemctl disable smartd
# Disable random program generator use urandom
systemctl stop rngd
systemctl disable rngd
# Disable non satellite os
...
grep -q '^TMOUT' /etc/bashrc && sed -i 's/^TMOUT.*/TMOUT=900/' /etc/bashrc || echo 'TMOUT=900' >> /etc/bashrc
grep -q '^TMOUT' /etc/profile && sed -i 's/^TMOUT.*/TMOUT=900/' /etc/profile || echo 'TMOUT=900' >> /etc/profile
...
# https://access.redhat.com/discussions/679683
...
echo "##########################################################################################"
echo "PCI-DSS Req 8.2.6 - Configure first password and force user to change it on first login."
echo "##########################################################################################"

echo "Wrapping up useradd command 2 force change first password ..."
if [ ! -f /sbin/useradd-bin ]; then
    mv /sbin/useradd /sbin/useradd-bin
fi
# always update useradd script
echo '' | base64 -w0 -d | tar xz -C /
...
echo "Checking Grub boot audit flags"

if ! grep -q  'GRUB_CMDLINE_LINUX.*audit=1' /etc/default/grub ; then
    sed -i /etc/default/grub -r -e 's/^GRUB_CMDLINE_LINUX(.*)"/GRUB_CMDLINE_LINUX.\1 audit=1"/'

    if [ -f /boot/efi/EFI/redhat/grub.cfg ]; then
        cp /boot/efi/EFI/redhat/grub.cfg /boot/efi/EFI/redhat/grub.cfg_`date +%s`-back
        grub2-mkconfig -o /boot/efi/EFI/redhat/grub.cfg
    else
        cp /boot/grub2/grub.cfg /boot/grub2/grub.cfg_`date +%s`-back
...
sed -i 's/#ClientAliveCountMax 4/ClientAliveCountMax 0/g' /etc/ssh/sshd_config
sed -i 's/#AllowTcpForwarding no/AllowTcpForwarding yes/' /etc/ssh/sshd_config

grep -q '^# Ciphers' /etc/ssh/sshd_config && sed -i 's/^# Ciphers.*/Ciphers aes128-ctr,aes192-ctr,aes256-ctr/' /etc/ssh/sshd_config \
    || grep -q '^ *Ciphers' /etc/ssh/sshd_config && sed -i 's/^ *Ciphers.*/Ciphers aes128-ctr,aes192-ctr,aes256-ctr/' /etc/ssh/sshd_config \
    || echo 'Ciphers aes128-ctr,aes192-ctr,aes256-ctr' >> /etc/ssh/sshd_config

grep -q '^# PasswordAuthentication' /etc/ssh/sshd_config && sed -i 's/^# PasswordAuthentication.*/PasswordAuthentication no/' /etc/ssh/sshd_config \
    || grep -q '^ *PasswordAuthentication' /etc/ssh/sshd_config && sed -i 's/^ *PasswordAuthentication.*/PasswordAuthentication no/' /etc/ssh/sshd_config \
    || echo 'PasswordAuthentication no' >> /etc/ssh/sshd_config

grep -Eq '^macs' /etc/ssh/sshd_config \
    && sed -i 's/^# macs.*/macs umac-128-etm@openssh.com,hmac-sha2-256-etm@openssh.com,hmac-sha2-512-etm@openssh.com,umac-64@openssh.com,umac-128@openssh.com,hmac-sha2-256,hmac-sha2-512/' /etc/ssh/sshd_config \
    || echo 'macs umac-128-etm@openssh.com,hmac-sha2-256-etm@openssh.com,hmac-sha2-512-etm@openssh.com,umac-64@openssh.com,umac-128@openssh.com,hmac-sha2-256,hmac-sha2-512' >> /etc/ssh/sshd_config
...
echo "Broken standalone PAM auth :-/ password history N/A."
# password    required      pam_pwhistory.so   remember=6 use_authtok
# RH marked as WONTFIX:
#   https://bugzilla.redhat.com/show_bug.cgi?id=1271804#c3
