...
import  {USER_TYPE_SUBSCRIBED, USER_TYPE_UNSUBSCRIBED} from '../app/constants';
    ...
    return (dispatch: Dispatch<any>, getState: Function) => {
        ...
        if (getState()['features/base/config'].enableWelcomePage) {
            setTimeout(
                () => {
                    let redirectUrl = '/';
                    let iamConfig = (getState()['features/base/config']).iam;
                    if (iamConfig){
                        const isGuest = (getState()['features/base/settings'] ||{}).userType !== USER_TYPE_SUBSCRIBED;
                        redirectUrl = `${iamConfig.iamUrl}${isGuest ? iamConfig.routes.congrats : iamConfig.routes.management}`;
                    }
                    dispatch(redirectWithStoredParams(redirectUrl));
                },
...
/**
 * Redirect kicked out participant .
 *
 * @returns {Function}
 */
export function redirectKickedOutParticipant() {
    return (dispatch: Dispatch<any>, getState: Function) => {
        let redirectUrl = '/';
        let iamConfig = (getState()['features/base/config']).iam;
        if (iamConfig){
