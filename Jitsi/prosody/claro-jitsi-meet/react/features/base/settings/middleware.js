...
import {getConferenceFromListStorage} from '../util/storage';
...
function _updateLocalParticipant({ dispatch, getState }, action) {
    ...
    dispatch(participantUpdated(appendCustomSettingsToParticipant(getState(), newLocalParticipant)));
}

/**
 * Adds participant data from local storage
 *
 * @param {Object} state - The redux store.
 * @param {Object} participant - Current participant data
 * @returns {Object}
 */
function appendCustomSettingsToParticipant(state, participant) {
    const conference = getConferenceFromListStorage(state);

    if (conference) {
        participant.virtualNumber = conference.virtualNumber;
        participant.admin = conference.admin;
    }

    logger.info(`[AMX DEBUG] Custom settings participant: ${JSON.stringify(participant)}`);
