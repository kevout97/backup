...
export function getInviteURL(stateOrGetState: Function | Object): string {
    ...
    let locationURL = false;
    const iamConfig = state['features/base/config'].iam;
    if (iamConfig) {
        const room = state['features/base/conference'].room;
        locationURL = `${iamConfig.baseUrl}${iamConfig.iamUrl}${iamConfig.routes.conference}/${room}`;
        return locationURL;
    }
    if (!locationURL) {
...
export function isInviteURLReady(stateOrGetState: Function | Object): boolean {
    const state = toState(stateOrGetState);
    return Boolean((state['features/base/config'] || {}).iam && (state['features/base/conference'] || {}).room);
}
