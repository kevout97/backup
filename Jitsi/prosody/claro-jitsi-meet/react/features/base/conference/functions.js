...
export function getConferenceName(stateful: Function | Object): string {
    ...
    const { conferenceSubject } = state['features/base/settings'];
...
export function getNearestReceiverVideoQualityLevel(availableHeight: number) {
    ...
    // TODO: [AMX DEBUG] Find a way to receive proper height
...
export function sendLocalParticipant(
    ...
    admin && conference.sendCommand('admin', {
        value: true
    });

    virtualNumber && conference.sendCommand('virtualNumber', {
        value: virtualNumber
    });
