...
export function conferenceFailedLeft() {
    return {
        type: CONFERENCE_FAILED_LEFT
    };
}
    ...
    return (dispatch: Dispatch<any>, getState: Function): ?Promise<void> => {
        ...
        case conference.lock: {
            ...
            if (state.conference === conference) {
                ...
                return sendPasswordToIAM(conference,password)
                        .then(res => {
                            return res.json()
                        })
                        .then((result) => {
                                ...
                                if (statusAPI) {
                                    ...
                                    const notificationA = showNotification({
                                        titleKey: 'info.passwordSuccess',
                                        appearance: NOTIFICATION_TYPE.SUCCESS,
                                        isDismissAllowed: true
                                    }, 5000);

                                    dispatch(notificationA);
                                    ...
                                    const notificationB = showNotification({
                                        titleKey: 'info.passwordFailed',
                                        appearance: NOTIFICATION_TYPE.ERROR,
                                        isDismissAllowed: true
                                    }, 5000);
                                    dispatch(notificationB);
                                    dispatch({
                                        type: SET_PASSWORD,
                                        conference,
                                        method,
                            ...
                            (error) => {
                                const notificationC = showNotification({
                                    titleKey: 'info.passwordFailed',
                                    appearance: NOTIFICATION_TYPE.ERROR,
                                    isDismissAllowed: true
                                }, 5000);
                                dispatch(notificationC);
                                dispatch({
                                    type: SET_PASSWORD,
                                    conference,
                                    method,
...
export function sendPasswordToIAM( conference: Object, password: string){

    let ENDPOINT;
    if(password == ''){
        ENDPOINT = 'delete-conference-password';
    }else{
        ENDPOINT = 'create-update-conference-password';
    }


    const apiToCall = window.config.iam.backendUrl + "/iam/v1/business/conference-lock/"+ENDPOINT;

    const params = {
        method: 'POST',
        credentials: 'include',
        body: JSON.stringify({
            conferenceId: conference.options.name,
            password: password,
        }),
        headers:{
