...
import {
    allowedSharingScreenPerms, askedAccessibilityPerms,
    askedSharingScreenPerms
} from '../../react/features/desktop-picker/actions';
...
import {hideDialog} from "../../react/features/base/dialog";
...
function initCommands() {
    commands = {
        ...
        /**
         * Events received for handling desktop event for special permissions
         */

        'allowed-screen-sharing-perms': (data) => {
            if (window && window.JitsiMeetScreenObtainer) {
                window.JitsiMeetScreenObtainer.onSourceChoose(data.id, data.type);
                APP.store.dispatch(hideDialog());
            }
        },

        'asked-for-screen-sharing-perms': () => {
            APP.store.dispatch(askedSharingScreenPerms());
        },

        'allowed-accessibility-perms': (data) => {
            APP.store.dispatch(hideDialog());
            APP.remoteControl.receiver.grant(data.participantId);
        },

        'asked-for-accessibility-perms': () => {
            APP.store.dispatch(askedAccessibilityPerms());
        },
...
class API {
     ...
     * @returns {void}
     */
    askForPermissions(type: string, data: Object) {
        switch (type){
            case 'screen-sharing':
                this._sendEvent({
                    name: 'ask-for-screen-sharing-perms', id: data.id, type: data.type
                });
            break;
            case 'accessibility':
                this._sendEvent({
                    name: 'ask-for-accessibility-perms', participantId: data.participantId
                });
        }
    }
