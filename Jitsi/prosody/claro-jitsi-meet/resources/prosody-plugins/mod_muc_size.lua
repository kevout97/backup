...
function handle_get_room_size(event)
    ...
    local domain_prefix = params["prefix"];

    local room_address
        = jid.join(room_name, domain_prefix.."."..domain_name);
		...
		if occupants then
			...
			for _, occupant in room:each_occupant() do
			    ...
			    if string.sub(occupant.nick,-string.len("/focus"))~="/focus" then
					...
					local admin = pr:get_child_text("admin") or "";
					local virtualNumber = pr:get_child_text("virtualNumber") or "";
					occupants_json:push({
					    jid = tostring(occupant.nick),
					    email = tostring(email),
					    admin = tostring(admin),
					    virtualNumber = tostring(virtualNumber),
