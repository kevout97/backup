...
ELASTIC_CONTAINER="elasticsearch-mast1.service"
...
ELASTIC_NODE_NAME="elasticsearch-ccmk"
...
ELASTIC_NETWORK_PUBLISH_HOST="10.23.142.134" # (Aqui va la Ip del Servidor) The publish host is the single interface that the node advertises to other nodes in the cluster, so that those nodes can connect to it. 
...
ELASTIC_HTTP_PUBLISH_HOST="10.23.142.134" # (Aqui va la Ip del servidor)	The host address to publish for HTTP clients to connect to.
...
ELASTIC_TRANSPORT_PUBLISH_HOST="10.23.142.134" # (Aqui va la Ip del servidor) The host address to publish for nodes in the cluster to connect to.
...
ELASTIC_DISCOVERY_SEDD_HOSTS="10.23.142.134, 10.23.142.133, 10.23.142.137" # Unicast discovery requires a list of hosts to use that will act as gossip routers.
ELASTIC_CLUSTER_INITIAL_MASTER_NODES="elasticsearch-ccmk, elasticsearch-nex"
