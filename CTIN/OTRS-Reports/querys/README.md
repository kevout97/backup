...
CREATE TABLE BaseJasper.reporte_actividades (proyecto VARCHAR(200),
                                             idGerencia INT,
                                             ...
                                             ticket_id BIGINT(20),
                                             responsable_first_name VARCHAR(100),
                                             responsable_last_name VARCHAR(100),
                                             tipo_actividad VARCHAR(200),
                                             fecha_inicio DATETIME);

DROP TABLE IF EXISTS BaseJasper.indicadores_rendimiento (name VARCHAR(500),
                                                        resumen_solicitudes INT,
                                                        resumen_solicitudes_porcentaje FLOAT,
                                                        cerrados_exitosamente INT,
                                                        cerrados_exitosamente_porcentaje FLOAT,
                                                        abandonados_usuario INT,
                                                        abandonados_usuario_porcentaje FLOAT,
                                                        validacion_usuario INT,
                                                        validacion_usuario_porcentaje FLOAT,
                                                        respuesta_usuario INT,
                                                        respuesta_usuario_porcentaje FLOAT,
                                                        seguimiento_terceros INT,
                                                        seguimiento_terceros_porcentaje FLOAT,
                                                        atencion_L2 INT,
                                                        atencion_L2_porcentaje FLOAT
                                                        );
...
INSERT INTO BaseJasper.reporte_actividades 
                        SELECT BaseJasper.propuestas_queues.gerencia,
                                ...
                                otrsdb.ticket.tn,
                                otrsdb.ticket.id,
                                otrsdb.users.first_name,
                                otrsdb.users.last_name,
                                otrsdb.service.name,
...
INSERT INTO BaseJasper.indicadores_rendimiento VALUES 
                                ("++ Solicitudes priorizadas con planeacion",20,58.82,13,38.24,1,2.94,0,0,1,2.94,1,2.94,4,11.76),
                                ("++ Solicitudes priorizadas sin planeacion",3,8.82,0,0,0,0,2,5.88,0,0,0,0,1,2.94);

INSERT INTO BaseJasper.indicadores_rendimiento
                                SELECT 
                                        "Subtotal solicitudes priorizadas",
                                        SUM(resumen_solicitudes),
                                        ROUND(SUM(resumen_solicitudes_porcentaje),2),
                                        SUM(cerrados_exitosamente),
                                        ROUND(SUM(cerrados_exitosamente_porcentaje),2),
                                        SUM(abandonados_usuario),
                                        ROUND(SUM(abandonados_usuario_porcentaje),2),
                                        SUM(validacion_usuario),
                                        ROUND(SUM(validacion_usuario_porcentaje),2),
                                        SUM(respuesta_usuario),
                                        ROUND(SUM(respuesta_usuario_porcentaje),2),
                                        SUM(seguimiento_terceros),
                                        ROUND(SUM(seguimiento_terceros_porcentaje),2),
                                        SUM(atencion_L2),
                                        ROUND(SUM(atencion_L2_porcentaje),2)
                                FROM 
                                        BaseJasper.indicadores_rendimiento;

INSERT INTO BaseJasper.indicadores_rendimiento VALUES 
                                ("Solicitudes no priorizados",11,32.35,1,2.94,1,2.94,0,0,1,2.94,0,0,8,23.53);
...
/*Falta el insert de los detalles de las solicitudes*/
...
SELECT gerencia, 
       ...
       sum(estado_application_for_business_risk_approval) as application_for_business_risk_approval,
       sum(estado_Queued_L2) as Queued_L2
       FROM gerencias_estados, propuestas_queues WHERE gerencias_estados.idGerencia = propuestas_queues.idGerencia AND gerencias_estados.idGerencia in (5,8,15,16,20,21,22,23) GROUP BY 1;
...
SELECT gerencia, 
       ...
       sum(estado_application_for_business_risk_approval) as application_for_business_risk_approval,
       sum(estado_Queued_L2) as Queued_L2
       FROM gerencias_estados, propuestas_queues WHERE gerencias_estados.idGerencia = propuestas_queues.idGerencia AND gerencias_estados.idGerencia in (28,29) GROUP BY 1; /*falta el id de Claro Musica*/
...
SELECT gerencia, 
       ...
       sum(estado_application_for_business_risk_approval) as application_for_business_risk_approval,
       sum(estado_Queued_L2) as Queued_L2
       FROM gerencias_estados, propuestas_queues WHERE gerencias_estados.idGerencia = propuestas_queues.idGerencia AND gerencias_estados.idGerencia = 18 GROUP BY 1;
